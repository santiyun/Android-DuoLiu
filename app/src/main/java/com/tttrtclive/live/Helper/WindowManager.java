...
public class WindowManager {
    ...
    public void add(long localId, long id, String deviceID, int oratation, int index) {
        for (int i = 0; i < mRemoteWindowList.size(); i++) {
            AudioRemoteWindow audioRemoteWindow = mRemoteWindowList.get(i);
            if (audioRemoteWindow.mIndex == index && !audioRemoteWindow.mDeviceID.equals(deviceID)) {
                audioRemoteWindow.hide();
                audioRemoteWindow.show(localId, id, deviceID, oratation);
                return;
            }
        }
    }
    ...
    public void addAndSendSei(long loginId, EnterUserInfo userInfo) {
        for (int i = 0; i < mRemoteWindowList.size(); i++) {
            ...
            if (audioRemoteWindow.mId == -1) {
                if (userInfo.mRole == CLIENT_ROLE_BROADCASTER)
                    audioRemoteWindow.show(userInfo.mUid, userInfo.mDeviceID);
                break;
            }
        }
    ...
    public void remove(String deviceID) {
        for (int i = 0; i < mRemoteWindowList.size(); i++) {
            AudioRemoteWindow audioRemoteWindow = mRemoteWindowList.get(i);
            if (audioRemoteWindow.mDeviceID.equals(deviceID)) {
                audioRemoteWindow.hide();
                return;
            }
        }
    ...
    public VideoCompositingLayout.Region[] buildRemoteLayoutLocation(long loginId) {
        ...
        for (AudioRemoteWindow remoteWindow : mRemoteWindowList) {
            if (remoteWindow.mId != -1) {
                ...
                mRegion.mDeviceID = remoteWindow.mDeviceID;
